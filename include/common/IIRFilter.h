...
public:
    ...
    /**
     * @brief   Reset filter blocks and state.
     */
    void reset();
...
private:
    ...
    /**
     * @brief   Calculate the filter gain over all the biquad cascades.
     */
    void calculate_total_gain();
...
template<uint32_t N, typename T, FilterPassType PASS_TYPE>
void IIRFilter<N, T, PASS_TYPE>::calculate_total_gain() {
    double gain = get_analog_gain();

    for (uint32_t i = 0; i < NUMBER_OF_BIQUAD_BLOCKS; ++i) {
        const auto &biquad_coefficients
                = reinterpret_cast<const BiquadCoefficients &>(
                        _cascade_filter.get_coefficients()[i * COEFFICIENTS_PER_BIQUAD_BLOCK]
                );
        const double biquad_gain = _pass_type_data.calculate_gain(biquad_coefficients);
        gain /= biquad_gain;
    }

    _cascade_filter.set_gain(gain);
}
...
T IIRFilter<N, T, PASS_TYPE>::process(U x) {
    if (_cascade_filter.update_coefficients_crossfade()) {
        calculate_total_gain();
    }
