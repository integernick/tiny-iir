...
template<size_t N, typename T, FilterPassType PASS_TYPE>
template<typename U, std::enable_if_t<std::is_same_v<U, double>, int>>
T IIRFilter<N, T, PASS_TYPE>::process(U *x, size_t num_samples) {
    return _cascade_filter.process(x, num_samples);
}

template<size_t N, typename T, FilterPassType PASS_TYPE>
const CascadeFilter<N, T> &IIRFilter<N, T, PASS_TYPE>::cascade_filter() const {
...
template<size_t N, typename T, FilterPassType PASS_TYPE>
void IIRFilter<N, T, PASS_TYPE>::configure(double normalized_cutoff_frequency) {
...
template<size_t N, typename T, FilterPassType PASS_TYPE>
std::complex<double> IIRFilter<N, T, PASS_TYPE>::transform(std::complex<double> s) {
    if constexpr (PASS_TYPE == FilterPassType::LOW_PASS) {
