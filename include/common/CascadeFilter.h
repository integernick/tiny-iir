...
public:
     ...
     * @return   The output sample.
     */
    template<typename U, typename std::enable_if_t<!std::is_same_v<T, U>> = 0>
     ...
     *
     * @return  True if the coefficients crossfade transition was updated, false otherwise.
     */
    bool update_coefficients_crossfade();
...
template<uint32_t N, typename T>
template<typename U, std::enable_if_t<!std::is_same_v<T, U>>>
...
template<uint32_t N, typename T>
bool CascadeFilter<N, T>::update_coefficients_crossfade() {
